...
class RawgService {
  ...
  Future<List<Game>> fetchGames({
    ...
    int daysToCache = 30,
    Duration cacheTTLHours = const Duration(hours: 24),
  }) async {
  ...
  Future<List<Game>> fetchTrending({
    ...
    int pageSize = 20,
    String ordering = '-added',
    Duration ttl = const Duration(hours: 1), // default TTL
    bool softTtl = true, // if true, serve cached immediately and refresh in background
    int minMetacritic = 60,
    int minRatingsCount = 50,
  }) async {
    final to = DateTime.now();
    final from = to.subtract(Duration(days: days));
    final dateStr = '${from.toIso8601String().split('T')[0]},${to.toIso8601String().split('T')[0]}';
    final cacheKey = 'trending|days=$days|genre=${genre ?? ''}|page=$page|size=$pageSize|ordering=$ordering';
    ...
    if (idList != null && idList.isNotEmpty && _isFresh(cacheKey, ttl)) {
      ...
      final assembled = await _assembleFromIds(idList, missingFetchBatch: 5, allowPartialReturn: false);
    ...
    if (softTtl && idList != null && idList.isNotEmpty) {
      final cachedGames = idList.map((id) => _gamesBox.get(id)).whereType<Game>().toList();
  ...
  Future<List<Game>> _fetchTrendingFromNetwork({
    ...
    final uri = Uri.https(_host, '$_basePath/games', params);
    final response = await httpClient.get(uri);
    if (response.statusCode != 200) throw Exception('Failed to fetch trending (status: ${response.statusCode})');
    ...
    final results = (data['results'] as List<dynamic>)
        ...
        .where((g) {
          ...
          if (g.metacriticRating != -1 && g.metacriticRating < minMetacritic) return false;
  ...
  Future<List<Game>> _assembleFromIds(
    ...
    if (missing.isNotEmpty) {
      for (var i = 0; i < missing.length; i += missingFetchBatch) {
        final batch = missing.sublist(i, (i + missingFetchBatch).clamp(0, missing.length));
