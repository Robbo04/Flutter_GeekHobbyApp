...
class _SuggestionsPageState extends State<SuggestionsPage> {
  ...
  Future<void> _fetchNextPage() async {
    final hasMore = _contentType == ContentType.games
        ? _gamesHasMore
        : _animeHasMore;
  ...
  Future<void> _fetchAnime() async {
    final fetched = await _anilist.fetchTrendingFranchises(
      page: _animePage,
      perPage: _pageSize,
    );
  ...
  void _onSkipPressed() {
    ...
    final name = _contentType == ContentType.games
        ...
        : (items.first as AnimeFranchise).title;
    _removeTop();
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          Expanded(
            child:
                (_contentType == ContentType.games ? _gameItems : _animeItems)
                    .isEmpty
                ? Center(
                    child: _isLoading
                ...
                : LayoutBuilder(
                    builder: (context, constraints) {
                      ...
                      final availableForTop =
                          constraints.maxHeight - minBottomSpace;

                      // Top area takes 50-55% of height, but respect minimum bottom space
                      final topAreaHeight = (constraints.maxHeight * 0.52)
                          .clamp(constraints.maxHeight * 0.45, availableForTop);
                      ...
                      final boxHeight =
                          (preferredHeight <= topAreaHeight * 0.88)
                          ? preferredHeight
                          : (topAreaHeight * 0.88);
                      ...
                      final double actionIconSize = (boxWidth * 0.10).clamp(
                        18.0,
                        32.0,
                      );
                                    ...
                                    child: SwipeCard(
                                      key: ValueKey(
                                        _contentType == ContentType.games
                                            ? _gameItems.first.id
                                            : _animeItems.first.franchiseId,
                                      ),
                                      width: boxWidth,
                                      height: boxHeight,
                                      onDrag: (off) =>
                                          setState(() => _cardOffset = off),
